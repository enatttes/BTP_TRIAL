CLASS zcl_handler_libros_enattesb DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS zcl_handler_libros_enattesb IMPLEMENTATION.
    ...
    CASE request->get_method( ).
      ...
      WHEN CONV string( if_web_http_client=>post ).
        handle_post( request  = request
                     response = response ).
      WHEN CONV string( if_web_http_client=>put ).
        handle_put( request  = request
                    response = response ).
      WHEN CONV string( if_web_http_client=>delete ).
        handle_delete( request  = request
                       response = response ).
      WHEN OTHERS.
...
* extract the filename and fileext from this metadata as well as the raw file data.
    ...
    LOOP AT content REFERENCE INTO content_item.  " put it all back together again humpdy dumpdy....
      IF content_item->* IS INITIAL.
        CONTINUE.
      ENDIF.
      IF content_item->*(1) EQ '['.
